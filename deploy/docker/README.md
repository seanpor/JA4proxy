...
| `deploy/docker/docker-compose.prod.yml` | Standalone | Production | Single-instance production stack; uses Docker secrets for credentials |
| `deploy/docker/docker-compose.monitoring.yml` | Overlay (joins POC networks) | Development / POC | Prometheus + Grafana + Loki + Alertmanager; attaches to a running POC stack |
...
docker compose -f deploy/docker/docker-compose.poc.yml \
               -f deploy/docker/docker-compose.python-legacy.yml \
...
```bash
docker compose -f deploy/docker/docker-compose.poc.yml \
               -f deploy/docker/docker-compose.scale.yml up -d
```
...
|------|-------------|---------------|
| `deploy/docker/docker-compose.poc.yml` | Yes | — |
| `deploy/docker/docker-compose.python-legacy.yml` | No | `deploy/docker/docker-compose.poc.yml` |
| `deploy/docker/docker-compose.scale.yml` | No | `deploy/docker/docker-compose.poc.yml` |
| `deploy/docker/docker-compose.test.yml` | Yes | — |
| `deploy/docker/docker-compose.prod.yml` | Yes | — |
| `deploy/docker/docker-compose.monitoring.yml` | No | Running POC stack (external network join) |
...
The production stack (`deploy/docker/docker-compose.prod.yml`) uses Docker secrets (files in
...
| Location | Rule |
|----------|------|
| `deploy/docker/` | All production Dockerfiles that do not own their own module directory |
...
`dockerfile.location=module` to distinguish them from `deploy/docker/` images in inventory
