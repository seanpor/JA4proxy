...
### Added
- **Git history rewrite — purge the 25 MB `ja4-tap` binary + `coverage.txt` history (Phase 811)**: closes issue #292. PR #291 (Phase 810) stopped tracking the committed `ja4-tap` binary and `coverage.txt`, but the blobs remained baked into history. Rewrote `main`'s history with `git-filter-repo` to strip both (all 8 historical `coverage.txt` revisions too), landing directly on `main` via force-push since a history rewrite has no meaningful PR diff. Along the way, GitHub's `GH007` leak-prevention flagged ~2,400 commits `filter-repo` necessarily re-hashes as newly exposing three legacy personal email addresses used before the project switched to the GitHub noreply address — fixed with a `--mailmap` pass remapping `seanpor@acm.org`/`seoriord@tcd.ie`/`sean@porreca.io` to `seanpor@users.noreply.github.com`, rather than disabling GitHub's protection. Landing required discovering and simultaneously disabling three independent GitHub protection layers (`enforce_admins`, a separate repository ruleset, and classic branch protection's `allow_force_pushes`, none of which `enforce_admins` alone exempts) — a gap in `AGENTS.md`'s documented override procedure, flagged as a follow-up. The `v2.0.0` tag was also force-pushed (its commit hash changed despite predating `ja4-tap`, because an earlier `coverage.txt` revision is its ancestor). Verified clean via a fresh independent clone, not just the local rewrite artifact. Reconciled 93 remote branches confirmed dead via merged-PR lookup (deleted); 14 branches with real unmerged commits were deliberately left untouched for their owners. See `docs/phases/complete/PHASE_811.md`.
- **Nightly performance/load regression detection (Phase 805)**: `make bench-macro` (`ja4p test benchmark`) was real, working tooling, but nothing ran it on a schedule — a PR that regressed hot-path latency or throughput passed CI cleanly. New `.github/workflows/nightly-benchmark.yml` runs it nightly, gates the result via a new `scripts/nightly_benchmark_gate.py` against a fresh `docs/reports/nightly_benchmark_baseline.json` (20% regression threshold, matching this repo's existing gating convention), and publishes the raw result to a rolling GitHub Release asset. A new host-side puller (`scripts/export_ci_benchmark_textfile.sh`, installed via systemd timer per `docs/runbooks/nightly_benchmark_regression.md`) pulls that asset into the self-hosted Prometheus/Grafana stack through node-exporter's textfile collector — chosen over `remote_write`/Pushgateway specifically to avoid opening any inbound path from GitHub Actions to the monitoring host (see `docs/decisions/ADR-207.md`). New Grafana dashboard `ci_benchmark_trend.json` and two new Alertmanager rules (`JA4ProxyNightlyBenchmarkStale`, `JA4ProxyNightlyBenchmarkRegression`).
  Two significant corrections made during implementation, verified against current code rather than trusting the written plan (see `docs/phases/complete/PHASE_805.md`'s "Design correction" notes): (1) the plan's originally-named baseline source, `docs/reports/benchmark_baseline.txt`, turned out to be an orphaned Phase-16 artifact for Python-proxy modules deleted when the Python proxy was removed — replaced with a new baseline JSON seeded from `docs/performance/benchmarks.md`'s already-published, git-SHA-tagged bridge-macro measurement instead. (2) `promtool check config` against the real Prometheus mount set uncovered that **none of the 10 files under `deploy/monitoring/alertmanager/rules/`** (proxy, redis, security, backup, tap, ebpf_attack, management_ui, slo_alerts, ti_feed, tls_alerts) were ever actually mounted into the Prometheus container, and `slo_recording_rules.yml` was referenced in `rule_files:` but never mounted either — Prometheus would not even start against the committed config. Both gaps are fixed; every alert rule in that directory is now genuinely evaluated for the first time, not just promtool-syntax-validated. Also closed matching validation gaps: `Makefile`'s `lint-prom`/`PROM_RULES` and `tests/unit/test_alert_rules.py`'s `RULE_FILES` now cover all rule files (the latter switched to a directory glob so it can't silently drift again); `lint-json`/`JSON_FILES` switched to a glob over `deploy/monitoring/grafana/dashboards/*.json`, picking up two more previously-unregistered dashboards (`04_capacity.json`, `ja4proxy-infrastructure.json`) alongside this phase's new one. See `docs/phases/complete/PHASE_805.md`.
- **Security-scan gate CVE fix + third-party image list derivation (Phase 810)**: fixed issue #364 (weekly scheduled `make scan` failure) — a newly-indexed CRITICAL CVE (`GHSA-r277-6w6q-xmqw`, a `kin-openapi` auth-bypass) in `grafana/grafana:13.0.2-ubuntu`, confirmed unfixed even in the latest published tag. Added a dated `.trivyignore` exception and bumped Grafana to `13.0.4-ubuntu` (18 fewer findings, zero new). While fixing the root cause, found that `make scan-images`'s third-party image list (`TRIVY_IMAGES`) was a hand-maintained Makefile variable that had drifted from `docker-compose.monitoring.yml`: one image's tag was stale, and three images (`tecnativa/docker-socket-proxy`, `gcr.io/cadvisor/cadvisor`, `prom/haproxy-exporter`) had been added to the compose file over time and were never added to the scan list, so they'd never been scanned at all. Between them they carried 113 real HIGH/CRITICAL findings. `scripts/check_image_versions.py` now derives that list directly from the compose files (`--list-third-party`, with a hard error on cross-file version drift instead of silently picking one version), so there is exactly one place a third-party image version is ever written down. Bumped `tecnativa/docker-socket-proxy` `0.3.0` → `v0.5.0` (now scans clean) and `gcr.io/cadvisor/cadvisor` `v0.47.2` → `v0.52.1` (57 → 41 findings); added dated `.trivyignore` justifications for what's left, including `prom/haproxy-exporter:v0.15.0` — confirmed abandoned upstream since 2023-02, no newer tag exists (a follow-up phase will evaluate replacing it with HAProxy 2.8's built-in Prometheus exporter). Also reviewed and landed PR #347 (Dependabot's grouped GitHub Actions bump) once `main` carried the CVE fix, after confirming its two breaking changes (`actions/checkout` v7's fork-PR checkout block, `actions/github-script` v9's `require()`/`getOctokit` changes) don't apply to any workflow in this repo. Issue #292 (git history rewrite to purge the `ja4-tap` blob) is explicitly deferred, not silently dropped. See `docs/phases/complete/PHASE_810.md`.
- **Go TAP/SPAN sensor remediation, Waves 1-2 (Phase 803)**: closed out the CRITICAL/HIGH findings from [[PHASE_334]]'s review of `cmd/ja4-tap`/`internal/tap/`. Verified every finding against current code rather than trusting the review's date: 5 of 10 Wave 1-2 findings (F-022 panic recovery, F-002 capability drop + seccomp, R-001 idle-poll timeout, F-023 metrics/`/health` endpoint, O-001 Makefile build target) turned out to already be fixed by an untracked prior commit that landed after the review but was never cross-referenced back — closed that gap by annotating `docs/phases/complete/PHASE_334.md` with resolution references instead of re-doing the work. Genuinely new in this phase: `internal/tap/sensor.go`'s read loop now distinguishes a benign idle-poll timeout from a genuine read error (`ErrPollTimeout` sentinel) and applies capped exponential backoff + a consecutive-error give-up threshold instead of busylooping at 100% CPU (F-014); a new `RedisCircuitBreaker` shared by `Store` and `Enforcer` skips writes after repeated Redis failures instead of stalling every event on a shared timeout (R-002); `cmd/ja4-tap`'s event buffer size is now a `--event-buffer` flag instead of hardcoded (R-004); 5 of 7 Alertmanager rules and 8 of 11 Grafana panels for the TAP sensor referenced metrics that never existed — both fixed and, just as importantly, added to the Makefile's `lint-prom`/`lint-json` gates so the drift can't recur silently (O-002, O-003). Waves 3-4 (MEDIUM/LOW, including the project's first privacy/GDPR documentation) are tracked separately as `docs/phases/PHASE_809.md` rather than dropped. See `docs/phases/complete/PHASE_803.md`.
- **`ja4proxy-management` base hardening, stage 2: SAML signature adversarial test (Phase 801, D2)**: added `management/tests/test_saml_signature_verification.py` — a real, non-mocked SAML assertion signature test. The existing `test_saml.py` mocks `OneLogin_Saml2_Auth` for every case, never exercising the actual `xmlsec`/`libxmlsec1` verification path python3-saml uses — exactly the code whose musl/Alpine build behavior the Phase 801 rebase needed to prove safe. Self-signs a minimal, spec-valid SAML 2.0 assertion with a freshly-generated RSA key/cert via `cryptography` + `OneLogin_Saml2_Utils.add_sign`, then covers three cases end-to-end through the real `/auth/sso/saml/acs` endpoint with `MANAGEMENT_SAML_STRICT=true`: a validly-signed assertion is accepted (positive control), the same assertion tampered post-signature (NameID changed without re-signing) is rejected with 401 (the acceptance-critical proof), and an assertion signed by an untrusted key is rejected with 401 (proves cert-pinning, not just signature presence). Verified passing both via the tools-image and directly inside the built Alpine `ja4proxy-management` image itself. `management/tests/`: 751 passed, 4 skipped, 0 failed with the new file included. Closes Phase 801. See `docs/phases/complete/PHASE_801.md`.
- **`management/tests/` wired into CI (Phase 808)**: `management/tests/` — 760+ tests including the real SAML/WebAuthn/TOTP/OIDC/RBAC/pentest-regression coverage, silently excluded from CI until Phases 806/807 fixed everything in it — now runs in `make test` and `make test-unit` as its own explicit step, matching `tests/unit/`'s pattern (kept separate rather than merged, since 4 filenames collide between the two directories: `test_attack_fingerprints.py`, `test_attack_mode.py`, `test_datacenter_policy.py`, `test_offense.py`). Also added to `pyproject.toml`'s `testpaths` for bare `pytest`/IDE runs. Found and fixed a real import-path bug while wiring this in: bare `pytest` failed with `ModuleNotFoundError: No module named 'management'` — `management/tests/` has `__init__.py` but `management/` itself doesn't (implicit namespace package), so pytest's rootdir walk-up stopped one level too early. Fixed with `pythonpath = ["."]` rather than touching application packaging. Added two guardrail tests to `tests/unit/test_makefile_targets.py` (one verified to actually catch the regression by simulation) so this can't silently recur. Documented the two-directory split in `docs/developer/TESTING_STRATEGY.md`. Closes the 806/807/808 CI-gap cleanup series found while scoping Phase 801. See `docs/phases/complete/PHASE_808.md`.
- **`ja4proxy-management` base hardening, stage 1: Alpine rebase (Phase 801)**: re-based `deploy/docker/Dockerfile.management` from Debian `python:3.14.0-slim` (41 HIGH/CRITICAL findings, 36 of them no-fix OS packages) to the same digest-pinned `python:3.14.6-alpine3.24` base already used by `analytics`/`tarpit`/`test`/`trafficgen` (Phase 317). All native deps (`cryptography`, `xmlsec`/`python3-saml`, `lxml`, `bcrypt`) resolved via prebuilt musllinux wheels — no source compilation needed. Verified with a full HTTP-level smoke test against the live built image (health check, CSRF token issuance, password login exercising `bcrypt` + JWT signing, an authenticated request, and the SAML metadata route executing cleanly) rather than just "the image builds." Fixed a pre-existing bug found in the process: the Dockerfile's own `HEALTHCHECK` had always pointed at `/health`, which doesn't exist — only `/api/v1/health` does; `docker-compose.poc.yml`'s own healthcheck override was masking this. Image size dropped 357MB → 204MB. Scans **0 HIGH/CRITICAL** (one dated `.trivyignore` residual, `CVE-2024-23342`/ecdsa, upstream won't-fix — tracked for the phase-304 PyJWT migration). `ja4proxy-management` is now gated by `scan-first-party`. See `docs/phases/complete/PHASE_801.md`; the SAML signature-tampering adversarial test (stage 2) is tracked separately, sequenced after Phases 806-808 land a real CI-verified safety net for `management/tests/`.
- **`management/tests/` schema-drift fixes + two real bug fixes (Phase 807)**: fixed all 27 remaining `management/tests/` failures (after Phase 806's dead-module cleanup). Most were genuine test-fixture drift — five files seeding the retired `ja4proxy:events` flat-schema stream key instead of `events:connection`'s ECS-dotted JSON, plus one Redis Stream XADD-id-collision test bug (descending ids, which Redis rejects). But investigating the failures surfaced two real, previously-undetected application bugs: (1) `get_fingerprint_detail`/`get_fingerprint_history` in `management/api/routes/connections.py` read raw flat stream fields instead of unwrapping the ECS-dotted `event` JSON the way `get_connections` does — these routes have likely never matched real Go-proxy traffic. The identical bug class was also found and fixed in **three GDPR-relevant `compliance.py` functions**: `_dsar_connection_history`/`_dsar_fingerprint_associations` (the actual GDPR Article 15 DSAR export — was silently returning empty connection history for every data subject access request) and `_aggregate_from_stream`/`_build_category_counts` (the executive compliance report — also compared against the wrong action string `"blocked"` instead of `"block"`, and read a nonexistent `signals` field instead of the ECS-dotted `ja4proxy.signals`). (2) `management/templates/partials/attack_table.html` already references `a.current_status`/`a.ban_expires`, confirming the "Under Attack" dashboard's ban-status column has been silently non-functional — implemented the missing ban-status lookup and block/ban/tarpit action tallying in `top_attackers()`. `management/tests/`: 35 → 0 failures. See `docs/phases/complete/PHASE_807.md`.
- **`management/tests/` dead-module test cleanup (Phase 806)**: removed 8 tests in `test_phase_122_security_review.py` that referenced Python modules deleted by earlier phases (`proxy.py`, `src.tap.enforcement_bridge`, `src.security.health`). Found while scoping Phase 801 that `management/tests/` (760 tests, including the real SAML/WebAuthn/OIDC/RBAC/pentest-regression coverage) is entirely excluded from `make test`. Investigation surfaced a likely genuine gap rather than a simple rename: the Go TAP sensor (`internal/tap/`, `cmd/ja4-tap/`) has no pub/sub subscription anywhere in its code, meaning the old `EnforcementBridge`'s role — consuming externally-published, HMAC-verified ban commands and fanning them out to iptables/BGP/webhook — was not carried forward when Python TAP mode was retired. Flagged for tracking, not resolved here. `management/tests/` failure count: 35 → 27. See `docs/phases/complete/PHASE_806.md`.
- **316\* series audit — Go TAP/SPAN sensor code review (Phase 334)**: a structured, review-only (no code changes, by design) audit of the entire Go TAP/SPAN passive sensor series (316a-316e) against its ADRs, tests, and manifest entries. Found unregistered during a phase-800 code-health session (2026-07-21) — written 2026-06-19, sat unactioned for a month. 40+ findings across correctness/security, reliability, TLS parsing, concurrency, observability, privacy/GDPR, and cross-component data integrity. Highlights: zero panic recovery (a single crafted packet permanently hangs the sensor — packet-of-death DoS that passes liveness checks while dead), capability drop + seccomp never wired (binary runs with full `CAP_NET_RAW` throughout its lifetime), SIGTERM ignored while blocked in a live packet read, no Makefile target builds the sensor at all, and the shipped Alertmanager rules/Grafana dashboard reference metric names that don't exist (8 of 11 dashboard panels show "No data", 5 of 7 alerts can never fire). None of the 40+ recommended fixes have been implemented — flagged for a dedicated remediation phase. See `docs/phases/complete/PHASE_334.md`.
- **Phase 15 gap closure — browser fixtures + container smoke test (Phase 502)**: added deterministic synthetic ClientHello fixtures mimicking real Chrome 120+/Firefox 122+ patterns (GREASE, `compress_certificate`, `post_handshake_auth`, multi-ALPN) to `tests/fixtures/clienthello/`, generated via a checked-in generator (`internal/tls/gen_browser_fixtures_test.go`) so they're reproducible, not hand-crafted. Also added container-config assertions (no Docker daemon required) verifying management/analytics wire to the Go proxy (`ja4pd`), not any deleted Python proxy service. Closes the two remaining open items from the Phase 15 post-implementation review. See `docs/phases/complete/PHASE_502.md`.
- **Root clutter follow-up (Phase 802)**: relocated 6 stray phase-notes files into `docs/phases/` (matching the convention 14 other phases already follow); deleted the orphaned `dc_head.yml` and the superseded Tailwind v3 pipeline at `management/tailwind/`; moved `.gitlab-ci/ja4proxy-policy.yml` (a customer-facing GitLab CI policy-as-code template, not this repo's own CI config) to `deploy/integrations/gitlab-ci/`, leaving a redirect stub at the old path for one release cycle. Critically reviewed and recommended **against** consolidating linter dotfiles into a `Dots`/`.config` folder — GitHub already hides dotfiles from its default view (so the visible-root-clutter argument that justified every other move doesn't apply), and 3 of the tools involved (`bandit`, `checkmake`, semgrep's ignore-file) rely on implicit cwd auto-discovery with no config-path override, risking a silently-narrower scan if moved. See `docs/phases/complete/PHASE_802.md`.
...
- **Under Attack Live Dashboard (Phase 247)**: adds `/under-attack` page with real-time top-attacker table (polled every 5s via HTMX), one-click 1h/24h IP ban buttons, and Attack Mode API (`POST/DELETE/GET /api/v1/attack-mode`) that raises the dial to 75 with a 4-hour auto-revert. See `docs/phases/complete/PHASE_247.md`.
- **Auto-escalating IP defense (Phase 248)**: IPs that persist after being tarpitted are automatically escalated to block then ban, without manual intervention. New `auto_escalate` config section (off by default) with configurable thresholds and shared-IP protection. Offense count visible in management API (`GET /api/v1/ip/{ip}/offense`). Attack Mode activates auto-escalation for its 4-hour duration via Redis key. Delivered as work packages Phase 248.1 (config schema), Phase 248.2 (Go offense counter), Phase 248.3 (pipeline integration), Phase 248.4 (management UI), Phase 248.5 (Attack Mode integration), and Phase 248.6 (tests) — all COMPLETE, tracked separately in `manifest.yaml` but not written up as separate entries here since they're one shipped feature. See `docs/phases/complete/PHASE_248.md`.
- **Datacenter ASN one-click block (Phase 249)**: Security Policy page lets operators tarpit or block all cloud/hosting provider traffic with one click — no config file editing, no restart. Hot-reloadable policy applies within seconds via Redis pub/sub. Default exceptions for Cloudflare, Fastly, and Akamai. Prometheus counter `ja4proxy_datacenter_policy_actions_total`. See `docs/phases/complete/PHASE_249.md`.
- **Botnet fingerprint detection (Phase 250)**: Fingerprint tab on Under Attack page groups attack traffic by JA4 — showing unique IP count, the key indicator of a residential botnet. One-click fingerprint block stops all botnet IPs simultaneously including future additions. Safety gate prevents blocking known-browser fingerprints (Chrome, Firefox, Safari) from the attack UI. Botnet signal detection (`detect_botnet_signal`) with configurable Attack Mode thresholds. Expanded JA4 corpus (15+ entries) with label file. See `docs/phases/complete/PHASE_250.md`.
...
- **Go TAP/SPAN sensor — capture & reassembly foundation (Phase 316a)**: new `internal/tap` package and standalone `cmd/ja4-tap` binary that read mirrored traffic (live AF_PACKET or offline `--pcap-file`), reassemble both directions of each TCP connection, and emit the exact ClientHello/ServerHello bytes of every TLS handshake as a `HandshakeEvent`. Pure-Go capture stack (`github.com/gopacket/gopacket` fork: `afpacket`, `pcapgo`, `reassembly`, zero-alloc `DecodingLayerParser`) — no cgo. Per-direction 16 KB reassembly cap plus global page ceiling bound memory under flood. No fingerprints, no Redis writes yet (that is 316b). See `docs/decisions/ADR-316a.md` and `docs/phases/complete/PHASE_316a.md`.
