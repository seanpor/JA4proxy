...
| `docs/phases/TODO.md`, `docs/PROJECT_STATUS.md` | READ ONLY — generated by `make sync`. On a merge conflict here, take either side then re-run `make sync` (they're derived from `manifest.yaml`; the conflict is mechanical). |
...
12. Commit code, the changelog fragment, `manifest.yaml`, `TODO.md`, and
    `PROJECT_STATUS.md` as one atomic commit.
