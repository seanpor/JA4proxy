...
| `docs/phases/TODO.md`, `docs/PROJECT_STATUS.md` | NOT COMMITTED — gitignored build artifacts generated from `manifest.yaml` by `make sync` (Phase 332). Never `git add` them; there is nothing to conflict on. Edit `manifest.yaml` instead. |
...
- `docs/PROJECT_STATUS.md` — high-level status, regenerated by `make sync`
...
See `docs/DOCUMENTATION_STANDARDS.md`. Summary:
