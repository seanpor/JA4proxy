...
async def _list_set(redis, key: str, page: int, per_page: int) -> PaginatedResponse:
...
async def list_blacklist(
    ...
    try:
        return await _list_set(redis, "ja4:blacklist", page, per_page)
...
async def add_to_blacklist(
    ...
    try:
        await redis.sadd("ja4:blacklist", payload.fingerprint)
...
async def remove_from_blacklist(
    ...
    try:
        removed = await redis.srem("ja4:blacklist", fingerprint)
        if not removed:
            raise HTTPException(
                status_code=404,
...
async def list_whitelist(
    ...
    redis = request.app.state.redis
    try:
        return await _list_set(redis, "ja4:whitelist", page, per_page)
    except redis.exceptions.RedisError as exc:
...
async def add_to_whitelist(
    ...
    try:
        await redis.sadd("ja4:whitelist", payload.fingerprint)
        await write_audit_log(
            redis,
...
async def remove_from_whitelist(
    ...
    try:
        is_member = await redis.sismember("ja4:whitelist", fingerprint)
        if not is_member:
            raise HTTPException(
                status_code=404,
                detail=f"Fingerprint {fingerprint!r} not in whitelist",
            )
        await redis.srem("ja4:whitelist", fingerprint)
        await publish_invalidation(redis, {
...
async def list_candidates(
    ...
    try:
        items, total = await get_fingerprint_candidates(
            redis, sort=sort, page=page, per_page=per_page
        )
    except redis.exceptions.RedisError as exc:
...
async def approve_candidate(
    ...
    try:
        score = await redis.zscore("ja4:candidates", fingerprint)
        if score is None:
            raise HTTPException(
                status_code=404,
        ...
        await redis.zrem("ja4:candidates", fingerprint)
        await publish_invalidation(redis, {
            "event": "ja4_blacklist_add",
            "value": fingerprint,
        })
