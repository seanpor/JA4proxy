...
.PHONY: all help build rebuild test test-unit test-chaos test-adversarial test-calibrate test-docker lint lint-static lint-docker lint-shell lint-yaml lint-prom lint-deps lint-secrets lint-go-full lint-lua lint-json lint-alertmanager scan-images scan-dockerfiles scan-first-party check-image-versions clean deploy-poc deploy-enterprise smoke-test smoke-docker smoke-k8s measure-mttr flush-redis attack-status top-attackers block-ja4 block-ip unblock-ip fetch-db list-pending approve-all geoip-report geoip-monitor geoip-watch update-geoip check-geoip start start-monitoring stop stop-clean status dial ssh-tunnels go-build go-test go-lint go-start go-stop go-switch go-rollback go-parity capture-fixtures go-build-ja4check test-go-docker test-go-integration test-go-chaos test-go-perf test-go test-go-redis-tls capture-fixtures-browser bench bench-quick bench-go bench-python test-tap test-tap-live test-tap-perf gdpr-delete lint-docs lint-phases link-check doc-health test-ratio agent-up agent-down agent-status management-build management-up management-down management-logs management-test management-shell tunnel test-infra-monitoring test-infra-monitoring-integration test-provisioning
...
help:
	...
	@echo "  test-component-suites - Run every individual component test suite"
...
.PHONY: verify-findings verify-findings-green findings-render findings-list verify-manifest-closeout
