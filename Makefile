...
lint-static: tools-image bandit-image
	...
	@# phase-827: the click/protobuf ignores that stood here are GONE. Their exit
	@# condition — semgrep shipping OTEL>=1.37 with mcp>=1.28 — was met by semgrep
	@# 1.173.0, so requirements.txt now pins through to the fixed versions and a
	@# regression fails this gate instead of being waived. See the note there.
	@$(TOOLS_RUN) bash scripts/pip-audit-resilient.sh -r requirements.txt \
	  ...
	  && echo "  ✓ pip-audit passed (remaining urllib3/pygments CVEs acknowledged — transitive deps, no safe resolution available)"
	@echo ""
