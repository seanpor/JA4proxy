...
help:
	...
	@echo ""
	@echo "── Lint aggregates (Phase 92) ───────────────────────────────"
	@echo "  lint-all          - Run every linter in one shot"
	@echo "  lint-python       - Python: lint-static + lint-security + lint-pylint"
	@echo "  lint-go           - Go: go-lint + lint-go-full + lint-go-mod"
	@echo "  lint-sast         - SAST: semgrep (cross-lang) + checkov (IaC)"
	@echo "  lint-infra        - Infra: Docker + shell + YAML + Lua + JSON + HAProxy + Makefile + TOML + Ansible + Helm"
	@echo "  lint-observability - Observability: Prometheus rules + Alertmanager"
	@echo "  lint-supply-chain - Supply chain: gitleaks + pip-audit + govulncheck"
	@echo "  lint-docs-all     - Docs: frontmatter + phases + links + markdown + spelling"
	@echo "  lint-pylint       - pylint errors-only (Python semantic bugs)"
	@echo "  lint-semgrep      - semgrep auto-config (cross-language patterns)"
	@echo "  lint-checkov      - checkov IaC security scan (Docker/Compose/Ansible/Helm)"
	@echo "  lint-haproxy      - haproxy -c syntax + semantic validation"
	@echo "  lint-helm         - helm lint chart validation"
	@echo "  lint-ansible      - ansible-lint playbook + role validation"
	@echo "  lint-markdown     - markdownlint structure (headings, lists, fences)"
	@echo "  lint-spelling     - codespell typo detection"
	@echo "  lint-toml         - TOML syntax validation (pyproject.toml, .gitleaks.toml)"
	@echo "  lint-makefiles    - checkmake Makefile validation"
	@echo "  lint-go-mod       - go mod verify module integrity"
	@echo "  test-phase-92     - Run Phase 92 lint hierarchy structural tests"
	@echo ""
