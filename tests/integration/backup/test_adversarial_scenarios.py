        ...
        for dangerous_filename in dangerous_filenames:
            ...
            try:
                ...
                if backup_file.exists():
                    ...
                    manifest_file = (
                        Path(self.backup_dir) / f"{dangerous_filename}.manifest.json"
                    )
                    manifest_file.write_text(json.dumps(manifest))
                    ...
                    with patch("src.backup.restorer.redis.Redis") as mock_redis_class:
                        ...
                        with pytest.raises((RestoreError, ValueError, OSError)):
                            restorer.restore_backup(
                                str(backup_file), str(manifest_file)
                            )
        ...
        with patch("src.backup.restorer.redis.Redis") as mock_redis_class:
            ...
            try:
                ...
                dangerous_calls = [
                    ...
                    if any(
                        dangerous in str(call)
                        for dangerous in ["eval", "system", "rm", "exploit"]
                    )
                ]
                assert (
                    len(dangerous_calls) == 0
                ), "No dangerous commands should be executed"
        ...
        try:
            ...
            with patch("src.backup.restorer.redis.Redis") as mock_redis_class:
                ...
                try:
                    ...
                    assert (
                        "permission" in str(e).lower() or "security" in str(e).lower()
                    )
