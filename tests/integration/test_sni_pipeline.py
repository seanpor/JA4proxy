...
class TestSNIPipelineIntegration:
    ...
    def pipeline(self):
        ...
        config = {
            "security_policy": {
                "alpn_browser_bypass": {
                    "enabled": False
                },  # Disable ALPN bypass for testing
    ...
    async def test_pipeline_with_unexpected_hostname(
        self, pipeline, connection_context
    ):
    ...
    async def test_pipeline_sni_analysis_disabled(self, pipeline, connection_context):
        ...
        for sni in test_cases:
            ...
            sni_signals = [
                s for s in result.signals if "sni" in s.name.lower() or s.name == "dga"
            ]
