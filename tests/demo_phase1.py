...
def print_metrics(results, scenario):
    ...
    for strategy, metrics in results.items():
        ...
        print(
            f"{icon} {strategy.value:20s}: {metrics.connections_per_second:3d} conn/sec"
        )
    print()
...
""")
    ...
    print_metrics(
        results, "Result: BY_IP strategy detects flood (15 > 10 threshold) 🚨"
    )
    ...
    print_metrics(
        results, "Result: BY_JA4 strategy detects botnet (30 > 25 threshold) 🚨"
    )
