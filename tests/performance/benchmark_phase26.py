...
"""
    ...
    async def run_benchmark(
        ...
        p90 = (
            latencies_sorted[int(len(latencies_sorted) * 0.9)]
            if latencies_sorted
            else 0
        )
        p99 = (
    ...
    async def run_security_validation(self, pipeline: Pipeline) -> Dict[str, bool]:
        ...
        try:
            ...
            validation["bypass_semantics_preserved"] = (
                result.action == "allow" and result.bypassed
            )
        except Exception:
    ...
    async def generate_report(self) -> CapacityReport:
        ...
        improvement = (
            result.connections_per_second / baseline_cps if baseline_cps > 0 else 0
        )
        ...
        self.report.notes = [
            ...
            (
                ...
                if self.report.security_validation["no_breaking_changes"]
    ...
    def print_report(self, report: CapacityReport) -> None:
        ...
        print(
            f"  Throughput: {report.single_process.connections_per_second:.0f} conn/s"
        )
        print(
            f"  Latency p50/p90/p99: {report.single_process.latency_p50:.2f}/{report.single_process.latency_p90:.2f}/{report.single_process.latency_p99:.2f} ms"
        )
        ...
        print(
            f"  Target (2.8×): {'✅ MET' if report.baseline_comparison['target_met'] else '❌ NOT MET'}"
        )
