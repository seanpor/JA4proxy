...
class TestRuntimeBenchmark:
    ...
    def test_small_dataset_backup_performance(self):
        ...
        assert (
            duration < 1.0
        ), f"Small dataset backup took {duration:.3f}s, expected < 1.0s"
    ...
    def test_medium_dataset_backup_performance(self):
        ...
        assert (
            duration < 3.0
        ), f"Medium dataset backup took {duration:.3f}s, expected < 3.0s"
    ...
    def test_large_dataset_backup_performance(self):
        ...
        mock_redis, keys = self._create_mock_redis_with_data(
            num_keys=2000, key_size=500
        )
        mock_access, mock_stat = self._mock_filesystem_validation()
        ...
        assert (
            duration < 10.0
        ), f"Large dataset backup took {duration:.3f}s, expected < 10.0s"
    ...
    def test_dataset_size_thresholds(self):
        ...
        for name, num_keys, total_size, duration in results:
            print(
                f"  {name:8}: {num_keys:4} keys, {total_size:7,} bytes, {duration:6.3f}s"
            )
        ...
        for name, num_keys, total_size, duration in results:
            ...
            assert (
                duration < 5.0
            ), f"{name} threshold took {duration:.3f}s, expected < 5.0s"

    def test_memory_efficiency(self):
        """Test that backup operations are memory efficient."""
        # Create a large dataset
        mock_redis, keys = self._create_mock_redis_with_data(
            num_keys=5000, key_size=1000
        )
        mock_access, mock_stat = self._mock_filesystem_validation()
        ...
        with patch("src.backup.worker.redis.Redis", return_value=mock_redis), patch(
            ...
            "pathlib.Path.write_bytes", side_effect=mock_write_bytes
        ), patch(
            "pathlib.Path.write_text"
        ):
        ...
        assert (
            duration < 15.0
        ), f"Large memory test took {duration:.3f}s, expected < 15.0s"

        print(
            f"✓ Memory efficiency test (5000 keys): {duration:.3f}s, max memory: {max_memory_used[0]:,} bytes"
        )
    ...
    def test_concurrent_backup_performance(self):
        ...
        ), f"Average sequential backup took {average_duration:.3f}s, expected < 2.0s"

        print(
            f"✓ Concurrent backup performance ({num_backups} backups): avg {average_duration:.3f}s"
        )
    ...
    def test_backup_overhead_analysis(self):
        ...
        for num_keys, total_size, duration, overhead in overheads:
            print(
                f"  {num_keys:3} keys, {total_size:6,} bytes: {duration:5.3f}s ({overhead:.6f}s/byte)"
            )
        ...
        for num_keys, total_size, duration, overhead in overheads:
            ...
            assert (
                overhead_per_kb < 1.0
            ), f"Overhead too high: {overhead_per_kb:.3f}ms/KB, expected < 1.0ms/KB"
    ...
    def test_performance_regression_detection(self):
        ...
        for name, (num_keys, key_size, baseline_duration) in baselines.items():
            ...
            if duration > regression_threshold:
                print(
                    f"⚠️  REGRESSION DETECTED: {name} dataset took {duration:.3f}s, expected < {regression_threshold:.3f}s"
                )
                regression_detected = True
            else:
                print(
                    f"✓ {name:6} dataset: {duration:.3f}s (baseline: < {baseline_duration:.1f}s)"
                )

        # Performance assertion: no regressions should be detected
        assert (
            not regression_detected
        ), "Performance regression detected in one or more test cases"
    ...
    def test_cold_vs_warm_start_performance(self):
        ...
        mock_redis1, keys1 = self._create_mock_redis_with_data(
            num_keys=300, key_size=500
        )
        mock_access, mock_stat = self._mock_filesystem_validation()

        start_time = time.time()

        with patch("src.backup.worker.redis.Redis", return_value=mock_redis1), patch(
            "os.access", side_effect=mock_access
        ), patch("os.stat", return_value=mock_stat), patch("pathlib.Path.mkdir"), patch(
            "pathlib.Path.exists", return_value=True
        ), patch(
        ...
        mock_redis2, keys2 = self._create_mock_redis_with_data(
            num_keys=300, key_size=500
        )

        start_time = time.time()

        with patch("src.backup.worker.redis.Redis", return_value=mock_redis2), patch(
            "os.access", side_effect=mock_access
        ), patch("os.stat", return_value=mock_stat), patch("pathlib.Path.mkdir"), patch(
            "pathlib.Path.exists", return_value=True
        ), patch(
        ...
        if cold_start_duration >= 0.05:
            assert (
                warm_start_duration < cold_start_duration * 2
    ...
    def test_dataset_size_vs_performance_curve(self):
        ...
        ]  # 100 to 1000 keys, 100 bytes each
        ...
        if len(durations) >= 2:
            ...
            print(
                f"  Performance scaling: {duration_ratio:.2f}x duration for {size_ratio}x data"
            )
