...
class TestHotPathNonRegression:
    ...
    def test_backup_does_not_block_connection_handling(self):
        ...
        avg_latency = (
            sum(connection_latencies) / len(connection_latencies)
    ...
    def test_backup_yields_to_hot_path(self):
        ...
        avg_hot_path = (
            sum(hot_path_completed) / len(hot_path_completed)
            if hot_path_completed
            else 0
        )
        ...
        assert (
            total_duration < 5.0
        ), f"Overall operation too slow: {total_duration:.3f}s"
    ...
    def test_backup_priority_inversion_prevention(self):
        ...
        with patch("src.backup.worker.redis.Redis"), patch("os.access"), patch(
            "os.stat"
        ...
        assert (
            avg_high_priority < avg_low_priority
    ...
    def test_backup_resource_contention(self):
        ...
        avg_contention = (
            sum(resource_wait_times) / len(resource_wait_times)
        ...
        assert (
            avg_contention < 0.1
        ), f"Excessive contention delay: {avg_contention:.3f}s"
    ...
    def test_backup_timeout_handling(self):
        ...
        assert (
            backup_completed or timeout_occurred
        ), "Backup should either complete or timeout gracefully"
    ...
    def test_backup_concurrency_limit(self):
        ...
        assert (
            avg_duration < 3.0
        ), f"Concurrent backups too slow: {avg_duration:.3f}s avg"
    ...
    def test_backup_cpu_usage(self):
        ...
        def sample_cpu_usage():
            ...
            cpu_usage_samples.append(
                0.3 + 0.1 * (len(cpu_usage_samples) % 3)
            )  # 30-50% usage
    ...
    def test_backup_io_pattern(self):
        ...
        with patch("src.backup.worker.redis.Redis", return_value=mock_redis), patch(
            ...
            "pathlib.Path.write_bytes", side_effect=mock_write_bytes_with_tracking
        ), patch(
            "pathlib.Path.write_text", side_effect=mock_write_text_with_tracking
        ):
    ...
    def test_backup_network_usage(self):
        ...
        with patch("src.backup.worker.redis.Redis") as mock_redis_class:
            ...
            mock_redis_instance.scan.side_effect = lambda cursor, **kwargs: (
                (0, keys) if cursor == 0 else (0, [])
            )
    ...
    def test_backup_hot_path_isolation(self):
        ...
        assert (
            avg_hot_path < avg_backup_path
        ), "Hot path should be faster than backup path"
        assert (
            avg_backup_path / avg_hot_path < 10
        ), "Backup path too much slower than hot path"
    ...
    def test_backup_priority_scheduling(self):
        ...
        avg_high = (
            sum(high_priority_times) / len(high_priority_times)
            if high_priority_times
            else 0
        )
        avg_medium = (
            sum(medium_priority_times) / len(medium_priority_times)
            if medium_priority_times
            else 0
        )
        avg_low = (
            sum(low_priority_times) / len(low_priority_times)
            if low_priority_times
            else 0
        )
