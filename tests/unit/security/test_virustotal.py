...
@pytest.mark.asyncio
async def test_virustotal_start_enabled_spawns_workers(mock_redis, mock_local_cache, mock_session):
...
@pytest.mark.asyncio
async def test_virustotal_start_loads_quota_from_redis(mock_redis, mock_local_cache, mock_session):
...
async def test_virustotal_get_signal_cache_hit_adaptive_cache_recorded(
    ...
    mock_local_cache.virustotal_scores.get.return_value = {"malicious_count": 1, "suspicious_count": 0}
...
@pytest.mark.asyncio
async def test_virustotal_maybe_lookup_redis_hit(mock_redis, mock_local_cache, mock_session):
    """Redis cache hit populates local cache and skips queue."""
    config = VirusTotalConfig(enabled=True, api_key="key", daily_quota=100)
    provider = VirusTotalProvider(config, mock_redis, mock_local_cache, mock_session)

    mock_redis.get.return_value = json.dumps({"malicious_count": 2, "suspicious_count": 0})
...
@pytest.mark.asyncio
async def test_virustotal_maybe_lookup_bloom_already_seen(mock_redis, mock_local_cache, mock_session):
...
@pytest.mark.asyncio
async def test_virustotal_maybe_lookup_queue_full_swallowed(mock_redis, mock_local_cache, mock_session):
    """QueueFull is silently swallowed — proxy must not crash on backpressure."""
    config = VirusTotalConfig(enabled=True, api_key="key", daily_quota=100, queue_size=1)
    provider = VirusTotalProvider(config, mock_redis, mock_local_cache, mock_session)
...
@pytest.mark.asyncio
async def test_virustotal_process_lookup_404_caches_zero(mock_redis, mock_local_cache, mock_session):
...
@pytest.mark.asyncio
async def test_virustotal_process_lookup_403_rate_limited(mock_redis, mock_local_cache, mock_session):
...
async def test_virustotal_process_lookup_uses_adaptive_ttl(
    ...
    config = VirusTotalConfig(enabled=True, api_key="key", daily_quota=100, cache_ttl_seconds=3600)
    provider = VirusTotalProvider(
        config, mock_redis, mock_local_cache, mock_session, adaptive_cache=adaptive_cache
    )
...
@pytest.mark.asyncio
async def test_virustotal_worker_loop_cancellation(mock_redis, mock_local_cache, mock_session):
...
@pytest.mark.asyncio
async def test_virustotal_stop_cancels_workers(mock_redis, mock_local_cache, mock_session):
...
@pytest.mark.asyncio
async def test_virustotal_worker_loop_processes_item(mock_redis, mock_local_cache, mock_session):
...
@pytest.mark.asyncio
async def test_virustotal_worker_loop_exception_logged(mock_redis, mock_local_cache, mock_session):
