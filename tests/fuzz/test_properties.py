...
class TestJA4FingerprintProperties:
    ...
    @given(
        st.lists(st.integers(min_value=0, max_value=0xFFFF), min_size=1, max_size=20)
    )
    ...
    def test_timestamp_validation(self, timestamp):
        ...
        try:
            fp = JA4Fingerprint(
                ja4="t13d1516h2_8daaf6152771_02713d6af862", timestamp=timestamp
            )
...
class TLSFingerprintStateMachine(RuleBasedStateMachine):
    ...
    @rule(
        ...
        cipher_suites=st.lists(
            st.integers(min_value=0, max_value=0xFFFF), min_size=1, max_size=50
        ),
        extensions=st.lists(
            st.integers(min_value=0, max_value=0xFFFF), min_size=0, max_size=30
        ),
    )
...
class TestSecurityValidationProperties:
    ...
    def test_header_validation(self, headers):
        ...
        assert all(
            not re.search(r"[\x00-\x1f\x7f]", value) for value in result.values()
        )
...
class TestMemorySafety:
    ...
    def test_large_input_handling(self, data):
        ...
        try:
            ...
            result = parser.parse_client_hello(
                None
            )  # Will return None for invalid input
