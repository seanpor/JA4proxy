...
| **Pipeline latency p99** | Fraction of `ja4proxy_pipeline_duration_seconds` observations in the `le="0.01"` bucket (per-connection scoring path, not proxied request) | 99% under 10 ms | 5m / 1h / 28d burn windows | [`runbooks/slo_latency.md`](runbooks/slo_latency.md) | `JA4ProxyLatencyFastBurn`, `JA4ProxyLatencySlowBurn` |
