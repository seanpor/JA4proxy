...
The multi-stage `deploy/docker/Dockerfile.go-proxy` produces an Alpine runtime image. Verify after changes:

```bash
docker build -f deploy/docker/Dockerfile.go-proxy -t ja4proxy-go:latest .
