...
docker compose -f deploy/docker/docker-compose.poc.yml run --rm test pytest tests/unit/ -v
docker compose -f deploy/docker/docker-compose.poc.yml run --rm test pytest tests/integration/ -v

# Run single test file
docker compose -f deploy/docker/docker-compose.poc.yml run --rm test pytest tests/test_proxy.py -v

# Run with coverage report
docker compose -f deploy/docker/docker-compose.poc.yml run --rm test pytest tests/ --cov=proxy --cov-report=term
```
...
docker compose -f deploy/docker/docker-compose.poc.yml logs -f backend
docker compose -f deploy/docker/docker-compose.poc.yml logs -f redis

# Restart service
docker compose -f deploy/docker/docker-compose.poc.yml restart proxy

# Rebuild images
docker compose -f deploy/docker/docker-compose.poc.yml build
docker compose -f deploy/docker/docker-compose.poc.yml build --no-cache
```
...
docker compose -f deploy/docker/docker-compose.poc.yml run --rm test bash
...
| Problem | Solution |
|---------|----------|
| Services won't start | Run `docker compose -f deploy/docker/docker-compose.poc.yml down -v` then `./scripts/start-poc.sh` |
| Port conflicts | Edit ports in `deploy/docker/docker-compose.poc.yml` |
| Tests failing | Run `./scripts/start-poc.sh` then `./scripts/smoke-test.sh` |
| Permission errors | Run `sudo chown -R $USER:$USER reports/` |
| Redis connection fails | Run `docker compose -f deploy/docker/docker-compose.poc.yml restart redis` |
...
| Docker compose (POC) | `deploy/docker/docker-compose.poc.yml` |
