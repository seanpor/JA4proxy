...
## ⚠ Network exposure model (read first)

Remote access needs **no special stack** — the normal `make start` already
...
- `scripts/start-poc.sh` runs `scripts/check_bind_address.py`, which **refuses to
  start** if `AGENT_BIND_IP` is a public address or `0.0.0.0`.

To test from a laptop, set `AGENT_BIND_IP` to the server's **private** LAN
address (RFC1918 / RFC4193) in `.env` before starting:

```bash
echo "AGENT_BIND_IP=192.168.1.50" >> .env   # the server's private LAN IP
```

Never expose these services on a public interface; if the server is
internet-facing, use the SSH tunnels below instead. In the URLs that follow,
`<SERVER_IP>` means that private `AGENT_BIND_IP`.
...
```bash
# 0. (once) point the stack at the server's PRIVATE LAN IP — see exposure model above
echo "AGENT_BIND_IP=<server-private-ip>" >> .env

# 1. On the proxy server, start the full stack (validates AGENT_BIND_IP)
make start
...
- **Network** — your laptop must be able to reach the proxy server's IP on
  ...
  (see below).
- **Proxy server started** with `make start` after setting `AGENT_BIND_IP` to
  the server's private LAN IP (default `127.0.0.1` is local-only)
...
## SSH tunnels (alternative to direct binding)
...
|---------|-------------|-----|
| `curl: (7) Connection refused` | Proxy not running, or port blocked by firewall | `make start` on server; check firewall |
