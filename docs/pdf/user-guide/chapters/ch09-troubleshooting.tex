...
\begin{table}[ht]
  ...
  \begin{tabularx}{\linewidth}{lXX}
    ...
    \code{YAML parse error} & Syntax error in \code{config/proxy.yml} & Validate with
      \code{python3 -c "import yaml; yaml.safe\_load(open('config/proxy.yml'))"} \\
    \midrule
    Build fails at pip install & Network issue or PyPI unavailable & Try
      \code{docker compose build --no-cache ja4proxy} with a working network connection \\
    \bottomrule
  \end{tabularx}
