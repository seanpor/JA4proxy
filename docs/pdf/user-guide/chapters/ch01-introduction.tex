...
The browser ALPN bypass is optional and \emph{off by default}. Any connection can
advertise \code{h2} or \code{h1} ALPN values — a bot can trivially set them — so
when the bypass is disabled (the default), browser-\emph{looking} traffic is scored
like any other connection. Real browser protection comes from the dial starting at
0 (monitor mode, nothing blocked), fail-open defaults, and the JA4 whitelist for
trusted fingerprints. If you want to enable the ALPN bypass, you can configure it,
but you should understand that bots can then use it to impersonate a real browser.
