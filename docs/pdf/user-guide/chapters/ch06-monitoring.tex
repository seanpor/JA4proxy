...
\begin{table}[ht]
  ...
  \begin{tabularx}{\linewidth}{llX}
    ...
    \midrule
    Grafana & \url{http://localhost:3001} & Dashboard, log exploration, alerting UI \\
    Prometheus & \url{http://localhost:9091} & Metrics storage and query (PromQL) \\
    Loki & \url{http://localhost:3100} & Log aggregation (queried via Grafana) \\
    Alertmanager & \url{http://localhost:9093} & Alert routing and silencing \\
    \bottomrule
  \end{tabularx}
  \caption{Observability service endpoints}
\end{table}

The proxy exposes its Prometheus metrics endpoint at \code{http://localhost:9090}.
...
Open \url{http://localhost:3001} and select the \emph{JA4proxy Overview} dashboard.
