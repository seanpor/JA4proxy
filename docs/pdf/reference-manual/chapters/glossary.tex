...
\begin{description}[leftmargin=0pt, style=nextline, itemsep=8pt]
  ...
  The state of the local in-process decision cache immediately after startup, or
  for a newly seen client (a client IP + JA4 fingerprint not seen recently). Cold
  cache state means there is no locally cached decision; the pipeline must execute
  in full (including Redis lookups and signal collection) to produce a decision for
  this client.
  ...
  The state of the local in-process decision cache when it contains a recent
  decision for a given client — keyed on the client IP together with its JA4
  fingerprint, so clients sharing a fingerprint do not share a decision. A warm
  cache hit means the pipeline can return a decision in under 100 microseconds
  without any Redis or signal collection overhead. Cache entries have asymmetric
  TTLs: ALLOW entries are cached for 30 minutes; every blocking action for 30
  seconds (ADR-003).
