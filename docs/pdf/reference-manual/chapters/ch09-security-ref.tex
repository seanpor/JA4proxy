...
minimisation), and it enforces a configurable hard floor below which browser traffic
can never be blocked (ALPN structural bypass).
...
\begin{description}
  \item[ALPN structural bypass] h2/h1 ALPN connections can never be blocked, regardless
    of score or dial setting. This is not a configurable threshold — it is structural.
    Even with dial=100 and all signals firing, a connection presenting h2 ALPN is allowed.
    Only if the operator explicitly disables this bypass does the structural protection
    lift.\sidenote{Browser traffic presenting h2 or h1 ALPN could theoretically score
    highly if, for example, the IP is on Spamhaus DROP and the browser is behind a
    compromised ISP. The bypass ensures this never causes a false positive.}
