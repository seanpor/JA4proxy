...
\addlinespace
\ja\ Proxy & Go\textsuperscript{*} & :8080 & TLS ClientHello sniffing; pipeline execution; allow/block/tarpit/ban decisions; Redis I/O \\
\addlinespace
...
\bottomrule
\end{tabular}
{\footnotesize\textsuperscript{*}The Go proxy (\code{cmd/proxy/}, \code{internal/})
was promoted to production in Phase~15 and is the only implementation that ships
in releases, Docker images, and Helm charts. The Python proxy (\code{proxy.py},
\code{src/security/}) is retained as an experimental prototyping surface for new
signal modules; production-runtime Python is limited to the Management API,
analytics node, and compliance reporting --- services that are not on the proxy
hot path.}
\end{table}
\end{fullwidth}
...
\section{The Go and Python Implementations}
\index{Go production proxy}
\index{Python prototyping surface}

\subsection{Go (Production Runtime)}
\index{Go proxy!production}

The Go proxy in \code{cmd/proxy/} and \code{internal/} (built to \code{bin/proxy}) is
the production runtime. It was promoted to production in Phase~15 and is the only
implementation that ships in releases, Docker images, Helm charts, and enterprise
documentation. The Go implementation uses \code{net/netip} for IP handling, a
\code{crypto/tls}-derived ClientHello parser, and goroutine-per-connection
concurrency. It targets 10--50$\times$ the throughput of the Python prototype.

\subsection{Python (Experimental Prototyping Surface)}
\index{Python proxy!experimental}

The Python implementation in \code{proxy.py} and \code{src/security/} is retained as
an experimental prototyping surface for new signal modules. Engineers prototype in
Python, prove the idea, then port it to Go before it ever touches real traffic.
Python services that are \emph{not} the proxy --- the Management API (FastAPI), the
analytics node, and the compliance reporting service --- remain production Python
because they are not on the connection-handling hot path.

\begin{notebox}[Implementation Status]
The Go proxy is the only path that handles production traffic. The Phase 200-series
hardening (Redis TLS, PROXY v2 with TLVs, default-credential removal, expanded
signal coverage) lands in the Go runtime first; Python is updated only when needed
for prototyping.
\end{notebox}
