...
docker compose -f deploy/docker/docker-compose.poc.yml exec redis redis-cli -a "${REDIS_PASSWORD}" set "config:dial" 50
...
docker compose -f deploy/docker/docker-compose.poc.yml exec redis redis-cli -a "${REDIS_PASSWORD}" set "config:dial" 0
docker compose -f deploy/docker/docker-compose.poc.yml restart proxy
```
