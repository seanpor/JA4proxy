# PHASE 314b — TAP OS-Mismatch MVP (first end-to-end value)

> **STATUS: PROPOSED — plan for review. No code until approved.**
> Sub-phase 2 of the TAP series. **Depends on 314a** (handshake events).
...
This phase makes the 314a sensor compute a **passive OS classification** from the
...
- 314a hands us `HandshakeEvent{client_ip, …, tcp_options, …}` per flow.
...
later, opt-in phase (314d). State this explicitly in the runbook.
...
- The 314a sensor writes `fp:os:ip:{ip}` as a canonical bare OS class (v4 & v6).
...
- TAP-originated bans / iptables / BGP enforcement → 314d. Exporters → 314e.
