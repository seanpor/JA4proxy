...
| PO.4 | Define and use criteria for software security checks | Security-check criteria recorded in the deployment checklist, the comprehensive security audit, the testing strategy (per-phase completion gate including FP corpus + chaos), and the severity rubric used for triage. The closure-verification doc records the gate that every phase must pass. | `docs/security/SECURITY_CHECKLIST.md`, `docs/developer/TESTING_STRATEGY.md`, `docs/security/CLOSURE_VERIFICATION.md`, `docs/security/SEVERITY_RUBRIC.md` | None |
