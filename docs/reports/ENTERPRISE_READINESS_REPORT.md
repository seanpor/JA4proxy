...
1. **Redis Authentication**: Password is auto-generated by start-poc.sh
...
4. **Secrets in Code**: Passwords in docker-compose.poc.yml
...
redis-cli -a "$REDIS_PASSWORD" SADD ja4:blacklist "t12d090909_bad_signature"

# View active blocks
redis-cli -a "$REDIS_PASSWORD" KEYS "ja4:block:*"

# Clear all blocks (emergency)
redis-cli -a "$REDIS_PASSWORD" KEYS "ja4:block:*" | xargs redis-cli -a "$REDIS_PASSWORD" DEL
