...
echo "------------"
if docker compose -f deploy/docker/docker-compose.poc.yml exec -T redis redis-cli -a "${REDIS_PW}" ping 2>/dev/null | grep -q PONG; then
    echo -e "  ✓ Redis is running"
    WHITELIST=$(docker compose -f deploy/docker/docker-compose.poc.yml exec -T redis redis-cli -a "${REDIS_PW}" SCARD ja4:whitelist 2>/dev/null)
    BLACKLIST=$(docker compose -f deploy/docker/docker-compose.poc.yml exec -T redis redis-cli -a "${REDIS_PW}" SCARD ja4:blacklist 2>/dev/null)
    BLOCKS=$(docker compose -f deploy/docker/docker-compose.poc.yml exec -T redis redis-cli -a "${REDIS_PW}" KEYS 'ja4:block:*' 2>/dev/null | wc -l)
    BANS=$(docker compose -f deploy/docker/docker-compose.poc.yml exec -T redis redis-cli -a "${REDIS_PW}" KEYS 'ja4:ban:*' 2>/dev/null | wc -l)
...
echo "   docker compose -f deploy/docker/docker-compose.poc.yml ps"
echo ""
echo "3. View logs:"
