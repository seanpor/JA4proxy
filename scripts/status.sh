...
redis_cmd() {
    docker compose -f deploy/docker/docker-compose.poc.yml $POC_FLAGS exec -T redis redis-cli -a "$REDIS_PASS" --no-auth-warning "$@" 2>/dev/null
}
...
check_service() {
    ...
    local status
    # shellcheck disable=SC2086
    status=$(docker compose -f deploy/docker/docker-compose.poc.yml $POC_FLAGS ps "$svc" --format '{{.Status}}' 2>/dev/null || true)
    ...
    if [ -n "$status" ]; then
        ...
        status=$(docker compose -f deploy/docker/docker-compose.monitoring.yml ps "$svc" --format '{{.Status}}' 2>/dev/null || true)
        if [ -n "$status" ]; then
             if echo "$status" | grep -qi "Up"; then
                ok "${label} (${status})"
            else
...
# Redis
# shellcheck disable=SC2086
if docker compose -f deploy/docker/docker-compose.poc.yml $POC_FLAGS exec -T redis redis-cli -a "$REDIS_PASS" --no-auth-warning ping > /dev/null 2>&1; then
...
# shellcheck disable=SC2086
if docker compose -f deploy/docker/docker-compose.poc.yml $POC_FLAGS ps redis --format '{{.Status}}' 2>/dev/null | grep -qi "Up"; then
