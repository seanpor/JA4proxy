...
def make_base_header(length):
    ...
    return (
        bytes.fromhex(f"16 03 01 00 {length:02x}")
        + bytes.fromhex("01 00 00")
        + struct.pack(">H", length - 5)
    )
...
hdr = bytes.fromhex(
    "16 03 01 00 40"  # TLS record: handshake, TLS 1.2, 64 bytes
...
grease_ciphers = bytes.fromhex(
    "00 0a"  # Cipher list length (10 bytes)
    "0a 0a"  # GREASE
    "1a 1a"  # GREASE
    "2a 2a"  # GREASE
    "3a 3a"  # GREASE
...
grease_full = (
    hdr[:-1]
    + struct.pack(">H", len(hdr) - 5 + len(grease_ciphers))
    + grease_ciphers
    + bytes.fromhex("00")
)
...
sni_ext = (
    bytes.fromhex("00 1a")
    + struct.pack(">H", len(max_sni) + 5)
    + bytes.fromhex("00")
    + struct.pack(">H", len(max_sni) + 3)
    + bytes.fromhex("00 00")
    + max_sni
)
sni_full = (
    hdr[:-1]
    + struct.pack(">H", len(hdr) - 5 + 2)
    + bytes.fromhex("00 02 c0 2b")
    + bytes.fromhex("00")
    + sni_ext
)
...
sni_null_ext = (
    bytes.fromhex("00 10")
    + struct.pack(">H", len(null_sni) + 5)
    + bytes.fromhex("00")
    + struct.pack(">H", len(null_sni) + 3)
    + bytes.fromhex("00 00")
    + null_sni
)
sni_null_full = (
