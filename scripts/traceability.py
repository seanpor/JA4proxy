#!/usr/bin/env python3
"""traceability.py — generate ``docs/reference/TRACEABILITY.md`` from phase docs.
...
clause, and emits ``docs/reference/TRACEABILITY.md`` as a sortable markdown table.

The schema lives in ``docs/developer/STYLE_GUIDE.md`` §6. Tagging is opt-in: only phases
...
OUTPUT_PATH = ROOT / "docs" / "reference" / "TRACEABILITY.md"
...
def render_document(reqs: Iterable[Requirement]) -> str:
    """Render the full ``docs/reference/TRACEABILITY.md`` body."""
    body = (
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Generate docs/reference/TRACEABILITY.md from phase docs."
    )
    parser.add_argument(
        "--check",
        action="store_true",
