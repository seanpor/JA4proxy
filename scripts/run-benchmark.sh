...
docker compose -f deploy/docker/docker-compose.poc.yml up -d --scale proxy=2 --no-recreate 2>/dev/null || true

# Get the container names/IPs for the scaled proxies
sleep 3
PROXY_IPS=$(docker compose -f deploy/docker/docker-compose.poc.yml ps -q proxy | xargs -I{} docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' {} 2>/dev/null | tr '\n' ' ')
...
docker compose -f deploy/docker/docker-compose.poc.yml up -d --scale proxy=4 --no-recreate 2>/dev/null || true
sleep 3
...
docker compose -f deploy/docker/docker-compose.poc.yml up -d --scale proxy=1 --no-recreate 2>/dev/null || true
