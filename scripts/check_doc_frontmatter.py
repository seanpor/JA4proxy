...
class DocumentValidator:
    ...
    def _validate_frontmatter(self, file_path: Path, frontmatter: dict) -> bool:
        ...
        if missing:
            self.errors.append(
                (file_path, f"Missing frontmatter: {', '.join(missing)}")
            )
        ...
        if "last_reviewed" in frontmatter:
            ...
            try:
                ...
                self.errors.append(
                    (file_path, f"Invalid last_reviewed date format: {date_str}")
                )
                valid = False
            ...
            try:
                ...
                if days_stale > 180:
                    self.warnings.append(
                        (file_path, f"Stale last_reviewed: {days_stale} days old")
                    )
                    if self.fix_mode:
                        frontmatter["last_reviewed"] = datetime.now().strftime(
                            "%Y-%m-%d"
                        )
                        self.files_fixed += 1
            except ValueError:
