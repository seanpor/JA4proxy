...
for svc in haproxy proxy redis backend tarpit; do
    if docker compose -f deploy/docker/docker-compose.poc.yml ps "$svc" --format '{{.Status}}' 2>/dev/null | grep -qi "Up"; then
        STATUS=$(docker compose -f deploy/docker/docker-compose.poc.yml ps "$svc" --format '{{.Status}}' 2>/dev/null)
        pass "$svc  ($STATUS)"
    else
...
docker compose -f deploy/docker/docker-compose.poc.yml exec -T redis redis-cli -a "$REDIS_PASS" --no-auth-warning ping > /dev/null 2>&1 \
...
BL=$(docker compose -f deploy/docker/docker-compose.poc.yml exec -T redis redis-cli -a "$REDIS_PASS" --no-auth-warning SCARD ja4:blacklist 2>/dev/null || echo 0)
WL=$(docker compose -f deploy/docker/docker-compose.poc.yml exec -T redis redis-cli -a "$REDIS_PASS" --no-auth-warning SCARD ja4:whitelist 2>/dev/null || echo 0)
if [ "${BL:-0}" -gt 0 ]; then
...
[ -f deploy/docker/docker-compose.poc.yml ]  && pass "docker-compose.poc.yml" || fail "docker-compose.poc.yml — missing"
