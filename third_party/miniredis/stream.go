...
// Fields is a map[string]string view of Values for convenience.
...
// newStreamEntry creates a StreamEntry with both Values and Fields populated.
func newStreamEntry(id string, values []string) StreamEntry {
	...
	return StreamEntry{ID: id, Values: values, Fields: fields}
}
...
func (s *streamKey) add(entryID string, values []string, now time.Time) (string, error) {
	...
	s.entries = append(s.entries, newStreamEntry(entryID, values))
	return entryID, nil
}
