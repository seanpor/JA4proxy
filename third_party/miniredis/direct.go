...
func (m *Miniredis) XLen(key string) (int, error) {
	return m.DB(m.selectedDB).XLen(key)
}

// XLen returns the number of entries in a stream.
func (db *RedisDB) XLen(key string) (int, error) {
	db.master.Lock()
	defer db.master.Unlock()
...
// XRange returns stream entries from start to stop (inclusive).
// Use "-" for the minimum ID and "+" for the maximum ID.
func (m *Miniredis) XRange(key, start, stop string) ([]StreamEntry, error) {
	return m.DB(m.selectedDB).XRange(key, start, stop)
}

// XRange returns stream entries from start to stop (inclusive).
func (db *RedisDB) XRange(key, start, stop string) ([]StreamEntry, error) {
	db.master.Lock()
	defer db.master.Unlock()
	...
	for _, e := range s.entries {
		if start != "-" && streamCmp(e.ID, start) < 0 {
			continue
		}
		if stop != "+" && streamCmp(e.ID, stop) > 0 {
			break
		}
