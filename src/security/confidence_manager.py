...
class ConfidenceManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info("confidence_manager | event=initialized | feed_count=%d", len(self._feeds))
    ...
    def _update_metrics(self, feed_name: str) -> None:
        ...
        effective_score = confidence.manual_override if confidence.manual_override is not None else confidence.accuracy_score
    ...
    async def record_validation(self, feed_name: str, is_true_positive: bool) -> None:
        ...
        if total > 0:
            ...
            if blended_score > old_score:
                _CONFIDENCE_ADJUSTMENTS.labels(feed_name=feed_name, direction="up").inc()
            elif blended_score < old_score:
                _CONFIDENCE_ADJUSTMENTS.labels(feed_name=feed_name, direction="down").inc()
    ...
    async def clear_manual_override(self, feed_name: str) -> None:
        ...
        if feed_name in self._feeds:
            ...
            logger.info(
                "confidence_manager | event=override_cleared | feed=%s",
                feed_name
            )
