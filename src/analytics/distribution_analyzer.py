...
class DistributionAnalyzer:
    ...
    async def check_distribution_shift(self) -> Optional[Dict[str, Any]]:
        ...
        shift_result = self._analyze_distribution_shift(
            current_baseline, historical_baseline
        )
        ...
        if shift_result["distribution_shift"]:
            ...
            self.logger.warning(
                "Distribution shift detected: ks_statistic=%.3f, p_value=%.4f",
                shift_result["ks_statistic"],
    ...
    def _analyze_distribution_shift(
        ...
        p_value = self._approximate_p_value(
            ks_statistic, current["event_count"], baseline["event_count"]
        )
    ...
    def _calculate_ks_statistic(
        self, current_dist: Dict[str, float], baseline_dist: Dict[str, float]
    ) -> float:
            ...
            if len(counts) >= 3:
                ...
                if counts_without_max1:
                    ...
                    if abs(max1_idx - actual_max2_idx) >= 2:
                        ...
                        if (
                            valley_count < max(counts) * 0.5
                        ):  # Valley is less than 50% of peak
