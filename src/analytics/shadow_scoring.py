...
class ShadowScoring:
    ...
    async def _store_shadow_scores(self):
        ...
        await self.redis.set(
            self.shadow_key, json.dumps(shadow_stats), ex=3600
        )  # 1 hour TTL

        self.logger.info(
            "Stored shadow scores: median=%.2f, count=%d",
            shadow_stats["median"],
            shadow_stats["count"],
        )
