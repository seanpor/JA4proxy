...
class MonitoringSystem:
    ...
    def __init__(self, redis_conn: redis.Redis, config: Dict[str, Any]):
        ...
        self.drift_detector = DriftDetector(
            redis_conn, config.get("drift_detection", {})
        )
        self.distribution_analyzer = DistributionAnalyzer(
            redis_conn, config.get("distribution_analysis", {})
        )
        self.shadow_scoring = ShadowScoring(
            redis_conn, config.get("shadow_scoring", {})
        )
    ...
    async def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        # Count active alerts
        alerts = await self.get_alerts()
        status["metrics"]["active_alerts"] = sum(
            1 for alert in alerts.values() if alert is not None
        )
