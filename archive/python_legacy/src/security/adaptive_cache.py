...
class AdaptiveCacheManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(
                "adaptive_cache | event=initialized | feed_count=%d", len(self._feeds)
            )
    ...
    async def save_state(self) -> None:
        ...
        try:
            ...
            await self._redis.setex(
                "ja4proxy:adaptive_cache:state", 86400, json.dumps(state)  # 24h TTL
            )
        except Exception as e:
    ...
    def _update_metrics(self, feed_name: str) -> None:
        ...
        if total > 0:
            hit_ratio = (
                (self._hit_count.get(feed_name, 0) / total) if total > 0 else 0.0
            )
    ...
    async def record_cache_miss(
        self, feed_name: str, old_value: Optional[dict], new_value: dict
    ) -> None:
        """
    ...
    def _adjust_volatility(self, feed_name: str) -> None:
        ...
        if new_ttl > old_ttl:
            ...
            _CACHE_TTL_ADJUSTMENTS.labels(
                feed_name=feed_name, direction="unchanged"
            ).inc()
    ...
    def get_cache_stats(self, feed_name: str) -> Dict:
        ...
        access_count = max(1, volatility.access_count)
        hit_ratio = (
            (self._hit_count.get(feed_name, 0) / access_count)
            if access_count > 0
            else 0.0
        )
