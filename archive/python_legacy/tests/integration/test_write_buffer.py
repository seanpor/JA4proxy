...
class TestWriteBuffer:
    ...
    async def test_write_buffer_initialization(self):
        ...
        buffer = WriteBuffer(
            mock_redis, flush_interval_ms=50, max_batch_size=500, max_queue_size=1000
        )
    ...
    async def test_write_buffer_enqueue_and_flush(self):
        ...
        buffer = WriteBuffer(
            mock_redis, flush_interval_ms=100, max_batch_size=10, max_queue_size=20
        )
    ...
    async def test_write_buffer_overflow(self):
        ...
        mock_redis = MagicMock(spec=redis.Redis)
        buffer = WriteBuffer(
            mock_redis, flush_interval_ms=100, max_batch_size=5, max_queue_size=10
        )
        ...
        for i in range(15):  # More than max_queue_size
            ...
            if i < 10:
                ...
                assert (
                    success is False
                ), f"item {i}: expected False (load-shed), got {success}"
