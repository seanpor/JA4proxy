...
class TestChaosScenarios:
    ...
    def test_redis_timeout_during_backup(self):
        ...
        with patch.object(
            BackupWorker, "enumerate_keys", side_effect=mock_enumerate_keys
        ), patch("os.access", side_effect=mock_access), patch(
            ...
            assert (
                "Redis timeout" in str(exc_info.value)
    ...
    def test_network_interruption_during_backup(self):
        ...
        *enumeration* (scan) does abort the backup, which is the behaviour tested here.
        ...
        mock_redis.scan.side_effect = Exception(
            "Network interruption: connection reset by peer"
        )
    ...
    def test_network_latency_during_backup(self):
        ...
        with patch("src.backup.worker.redis.Redis", return_value=mock_redis), patch(
            ...
            with patch("pathlib.Path.write_bytes") as mock_write_bytes, patch(
                "pathlib.Path.write_text"
            ) as mock_write_text:
