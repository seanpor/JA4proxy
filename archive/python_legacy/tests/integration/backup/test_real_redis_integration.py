...
class TestRealRedisIntegration:
    ...
    def test_end_to_end_backup_restore(self):
        ...
        for key, expected_value in self.test_data.items():
            ...
            assert (
                restored_value.decode("utf-8") == expected_value
        ...
        assert (
            new_key_value is not None
        ), "New key should still exist after non-destructive restore"
    ...
    def test_backup_with_various_data_types(self):
        ...
        self.redis_client.hset(
            "hash_key", mapping={"field1": "value1", "field2": "value2"}
        )
        ...
        assert (
            self.redis_client.lindex("list_key", 1).decode("utf-8") == "item1"
        )  # item2, item1 (LPUSH order)
