...
def mock_redis():
    ...
    def zadd_impl(key, mapping):
        ...
        for member, score in mapping.items():
            # Remove if exists
            redis._sorted_sets[key] = [
                (m, s) for m, s in redis._sorted_sets[key] if m != member
            ]
            # Add new
    ...
    def zremrangebyscore_impl(key, min_score, max_score):
        ...
        redis._sorted_sets[key] = [
            (m, s) for m, s in redis._sorted_sets[key] if not (min_val <= s <= max_val)
        ]
...
class TestEndToEndBlockTraffic:
    ...
    async def test_block_high_rate_traffic(self, security_manager):
        ...
        assert (
            last_allowed is False
        ), f"High rate traffic should be blocked: {last_reason}"
        # The reason might contain "ban" instead of "tarpit" or "block"
        assert any(
            word in last_reason.lower() for word in ["tarpit", "block", "limit", "ban"]
    ...
    async def test_subsequent_connection_blocked(self, security_manager):
        ...
        assert (
            "block" in reason.lower()
            or "tarpit" in reason.lower()
            or "ban" in reason.lower()
        )
...
class TestMultiStrategyIntegration:
    ...
    async def test_different_strategies_different_thresholds(
        self, security_manager, mock_redis
    ):
...
class TestGDPRIntegration:
    ...
    async def test_enforcement_data_stored_with_gdpr(
        self, security_manager, mock_redis
    ):
...
class TestIntegrationEdgeCases:
    ...
    async def test_exactly_at_threshold(self, security_manager):
        ...
        for i in range(5):
            ...
            if i < 5:
                assert (
                    allowed is True
                ), f"Connection {i+1} at threshold should be allowed: {reason}"
    ...
    async def test_rapid_succession_same_client(self, security_manager):
        ...
        ), f"Not all connections should be allowed in rapid succession. Results: {[(i, r[0], r[1][:50]) for i, r in enumerate(results)]}"
