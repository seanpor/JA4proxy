    ...
    try:
        ...
        for i in range(3):
            ...
            manifest_file = os.path.join(
                backup_dir, f"backup_2026032{i+1}T160000Z.bin.manifest.json"
            )
...
def test_list_command_with_custom_directory():
    ...
    try:
        ...
        manifest_file = os.path.join(
            custom_dir, "backup_20260321T160000Z.bin.manifest.json"
        )
        ...
        mock_config_loader = MagicMock()
        mock_config_loader._read_and_parse.return_value = {
            "backup": {"destination": "/tmp/wrong_directory"}
        }
...
def test_list_command_missing_directory():
    ...
    mock_config_loader = MagicMock()
    mock_config_loader._read_and_parse.return_value = {
        "backup": {"destination": "/tmp/nonexistent_backup_directory"}
    }
...
def test_validate_command_corrupted_manifest():
    ...
    try:
        ...
        mock_restorer.load_manifest.side_effect = RestoreError(
...
def test_validate_command_checksum_failure():
    ...
    try:
        ...
        with patch.object(cli, "restorer", mock_restorer):
            ...
            assert result == 1
            mock_restorer.verify_checksum.assert_called_with(
                backup_file, wrong_checksum
            )
...
def test_list_validate_cli_argument_parsing():
    ...
    try:
        ...
        full_manifest_path = os.path.join(
            backup_dir, "backup_20260321T160000Z.bin.manifest.json"
        )
        ...
        with patch.object(cli, "restorer", mock_restorer), patch.object(
            ...
            with patch.object(
                sys, "argv", ["backup-cli", "validate", full_backup_path]
            ):
                result = cli.run()
                assert result == 0
            ...
            with patch.object(
                sys, "argv", ["backup-cli", "list", "--directory", backup_dir]
            ):
                result = cli.run()
                assert result == 0
