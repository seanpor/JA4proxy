...
def test_control_keys_success_path():
    ...
    with patch("src.backup.worker.redis.Redis", return_value=mock_redis), patch(
        ...
        success_calls = [
            call for call in set_calls if call[0][0] == "backup:last_success"
        ]
...
def test_control_keys_failure_path():
    ...
    with patch("src.backup.worker.redis.Redis", return_value=mock_redis), patch(
        ...
        "pathlib.Path.write_bytes", side_effect=OSError("No space left on device")
    ), patch(
        "pathlib.Path.write_text"
    ) as mock_write_text:
        ...
        failure_calls = [
            call for call in set_calls if call[0][0] == "backup:last_failure"
        ]
